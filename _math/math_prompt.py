...
COT = {
    ...
    thinking, answer = await cot_agent([taskInfo], cot_instruction)
    return answer
"""
}
...
Take_a_step_back = {
    ...
    thinking, answer = await solve_agent([taskInfo, principle], solve_instruction)
    return answer
"""
}
...
all_answers = []
for agent in agents:
    outputs = await agent([taskInfo], instruction)
